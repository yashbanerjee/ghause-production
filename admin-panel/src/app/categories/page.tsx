...
import { Plus, Edit, Trash2, X, Image as ImageIcon, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function CategoriesPage() {
    ...
    const fetchCategories = useCallback(async () => {
        setFetching(true);
        try {
            const { data } = await api.get(`/categories?page=${page}&limit=10&search=${searchTerm}`);
    ...
    const handleToggleStatus = async (id: string, currentStatus: boolean) => {
        try {
            ...
            setCategories(categories.map(cat => cat.id === id ? { ...cat, isActive: !currentStatus } : cat));
        } catch (err) {
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 whitespace-nowrap shadow-md shadow-indigo-100 transition-all"
                    >
                                ...
                                categories.map((cat) => (
                                    <tr key={cat.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                                            {cat.icon ? (
                                                <img 
                                                    src={cat.icon.startsWith('http') ? cat.icon : `${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:5000' : '')}/uploads/${cat.icon}`} 
                                                    alt="" 
                                                    className="h-10 w-10 object-contain p-1 bg-gray-50 rounded" 
                                                />
                                            ) : (
                                                <div className="h-10 w-10 bg-gray-50 rounded flex items-center justify-center border border-gray-100"><ImageIcon className="h-5 w-5 text-gray-300" /></div>
                                            )}
                                        </td>
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                            <div className="flex items-center justify-end gap-3">
                                                ...
                                                <button onClick={() => handleEdit(cat)} className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all border border-transparent hover:border-indigo-100">
                                                    <Edit className="h-4 w-4" />
                                                </button>
                                                <button onClick={() => handleDelete(cat.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-all border border-transparent hover:border-red-100">
                    ...
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                disabled={page === 1}
                                className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-xs font-bold text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                            >
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl bg-white p-8 shadow-2xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="text-left">
                                        ...
                                        <textarea
                                            value={formData.homeDescriptionEn}
                                            onChange={(e) => setFormData({ ...formData, homeDescriptionEn: e.target.value })}
                                    ...
                                    <div className="text-left text-right">
                                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest">Arabic</label>
                                        <textarea
                                            value={formData.homeDescriptionAr}
                                            onChange={(e) => setFormData({ ...formData, homeDescriptionAr: e.target.value })}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="text-left">
                                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest">English</label>
                                        <textarea
                                            value={formData.productPageDescriptionEn}
                                            onChange={(e) => setFormData({ ...formData, productPageDescriptionEn: e.target.value })}
                                            className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                            rows={4}
                                        />
                                    </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                                <div className="text-left">
                                    <label className="block text-sm font-medium text-gray-700">Icon (SVG/PNG)</label>
                                    <input
                                        type="file"
                                        accept="image/*"
                                        onChange={(e) => setIconFile(e.target.files?.[0] || null)}
                                        className="mt-1 block w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer"
                                    />
                                </div>
                                <div className="text-left">
                            ...
                            <div className="mt-8 flex justify-end gap-3 pt-6 border-t">
                                <button type="button" onClick={() => setShowModal(false)} className="rounded-lg border border-gray-300 px-6 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-all font-bold tracking-widest uppercase text-xs">Cancel</button>
                                <button disabled={loading} type="submit" className="rounded-lg bg-indigo-600 px-8 py-2 text-sm font-bold text-white hover:bg-indigo-700 disabled:bg-indigo-400 shadow-lg shadow-indigo-100 transition-all tracking-widest uppercase text-xs">
